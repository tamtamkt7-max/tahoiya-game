...
for (const key of requiredTopLevel) if (!(key in state)) throw new Error(`STATE_INVALID missing:${key}`);
...
if (state.validation.last_result === 'fail' && state.repair_budget.attempts_used >= state.repair_budget.max_attempts) throw new Error('SAFETY_STOP repair_budget_exhausted');
...
if (fs.existsSync(path.join(projectRoot, 'package-lock.json'))) run('npm', ['ci', '--no-audit', '--no-fund']);
else run('npm', ['install', '--no-audit', '--no-fund']);

for (const check of state.validation.required_checks) {
  if (check === 'state') continue;
  if (!pkg.scripts?.[check]) { console.error(`MISSING_REQUIRED_SCRIPT ${check}`); process.exit(2); }
